...
def _get_good_faa_layout_iteration(G,faa,faa_dict,count,suspensions,weights,outer_face):
	...
	count += 1
	if weights == None:
		sol = _get_plotting_matrix_iteration(G,suspensions,faa_dict,count)
		pos = {V[i]:sol[i] for i in range(n)}
		G.set_pos(pos)
		weights2 = _calculate_weights(G,faa,faa_dict,suspensions,count,outer_face)
	else:
...
def _calculate_weights(G,faa,faa_dict,suspensions,count,outer_face):
	## Calculates the lambda for one step in Approach 1
	V = G.vertices()
	n = len(V)
	W = zero_matrix(RR,n,n)
	W = _weights_for_pseudo_segments(G,faa,faa_dict,outer_face,suspensions,W)
	W = _weights_for_edges(G,faa,faa_dict,W)
	return W

def _weights_for_edges(G,faa,faa_dict,W):
	V = G.vertices()
	n = len(V)
	pos = G.get_pos()
	for face in G.faces():
...
def _weights_for_pseudo_segments(G,faa,faa_dict,outer_face,suspensions,W):
	V = G.vertices()
	pos = G.get_pos()
	x = 1
	for seg in _list_pseudo_segments(G,faa,faa_dict):
		...
		if vol_l1 != 0 and vol_r1 != 0:
			vol_l = (vol_l1^x)*(c_r)^1
			vol_r = (vol_r1^x)*(c_l)^1
			for node in seg+R:
				j = V.index(node)
				for n in R:
...
def _nodes_45_left_right(G,edge,pos):
	V = copy(G.vertices())
	pos = copy(G.get_pos())
	e0 = edge[0]
	e1 = edge[1]
	...
	x0 = pos[i0][0]
 	...
 	x_n = x/norm
 	y_n = y/norm
 	A = Matrix([[x_n,y_n],[-y_n,x_n]])
 	L = [[],[]]
 	for v in V:
		i = V.index(v)
		vec = A*(vector([pos[i][0],pos[i][1]])-vec0)
		if vec[1] < 0:
			if vec[0] < -vec[1]+norm and vec[0] > vec[1]:
				L[0].append(v)
		if vec[1] > 0:
			if vec[0] < vec[1]+norm and vec[0] > -vec[1]:
				L[1].append(v)
...
def _calculate_position_iteratively_edges(G,faa,faa_dict,suspensions,outer_face,weights=None):
	## Calculates the lamda for the second or the combined approach
	count = 0
	V = G.vertices()					
	n = len(V)
